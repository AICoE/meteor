...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    console.log(error);
    res.status(error?.statusCode || HttpStatusCode.SERVER_ERROR);
    res.json(error?.body || { message: error });
    httpRequestsTotal.labels({ method, statusCode: error?.statusCode || HttpStatusCode.SERVER_ERROR, path: '/api/meteors' }).inc();
  }
};
